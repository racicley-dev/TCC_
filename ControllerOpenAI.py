...
class ControllerOpenAI:
    ...
    def get_embedding(self,text: str, model="text-embedding-3-small", **kwargs) -> List[float]:
    ...
    async def aget_embedding(self, text: str, model="text-embedding-3-small", **kwargs) -> List[float]:
        ...
        return (await self.client.embeddings.create(input=[text], model=model, **kwargs))[
            "data"
        ][0]["embedding"]
    ...
    def indices_of_furtherest_neighbors_from_distances(self,distances) -> np.ndarray:
