...
if __name__ == "__main__":
    ...
    lista_distancias = ['cosine','L1','L2','Linf']
    for distance in lista_distancias:
        ra = RecomendArticle('csv_buscas_unificado_ajustado_final.csv',  readed_article)
        article_source, articles, index_near, index_futher, embeddings = ra.print_recommendations_from_strings(10,distance=distance)
        ...
        df.to_csv('datasets/Cannabis/'+distance+"_10x.csv")
        ...
        doi = [article['doi'] for article in articles]

        # Gera o gráfico com base nos embeddings
        labels = [f"Prioridade {article['article_priority']}" for article in articles]
        strings = [article['abstract'] for article in articles]
        chart = controller.chart_from_components(doi, components, labels=labels, strings=strings)
        
        if(distance in ['L1', 'L2', 'cosine']):
            chart.update_layout(
        ...
        # Exibe o gráfico
        #chart.show()
        chart.write_image('img/Cannabis/'+distance+'.png')